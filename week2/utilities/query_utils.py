...
def add_click_priors(query_obj, user_query, priors_gb):
    try:
        ...
        if prior_clicks_for_query is not None and len(prior_clicks_for_query) > 0:
            total_queries = len(prior_clicks_for_query)
            sku_clicks_counts = prior_clicks_for_query.groupby('sku')['sku'].count().nlargest(100)
            click_prior_query_obj = ' '.join([f'{sku}^{count/total_queries}' for (sku, count) in sku_clicks_counts.iteritems()])
            print(click_prior_query_obj)
            query_obj["query"]["function_score"]["query"]["bool"]["should"].append({
                "query_string": {
                    "query": click_prior_query_obj,
                    "fields": ["sku.keyword"]
                }
            })
